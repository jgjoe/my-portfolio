    ...
    featured: [
      {
        ...
        details: [
          ...
          { label: { ko: "기여", en: "Ownership" }, copy: { ko: "generic event-log 계약과 CSV/XES 브라우저 온보딩, DatasetRegistry/readiness를 구현했습니다. 공유 CoreReadSurface와 DatasetResolver 위에 FastAPI/React, 정확히 5개의 read-only 도구, Direct Agent와 stdio MCP를 연결하고 BPIC12 Power BI export는 별도 역사적 분석 표면으로 유지했습니다.", en: "I implemented the generic event-log contract, CSV/XES browser onboarding, and DatasetRegistry/readiness, then connected FastAPI/React, exactly five read-only tools, Direct Agent, and stdio MCP through the shared CoreReadSurface and DatasetResolver. The BPIC12 Power BI export remains a separate historical analytical surface." } },
          { label: { ko: "판단", en: "Decision" }, copy: { ko: "AI는 authoritative metric을 계산하지 않고 5개 read-only 도구가 반환한 fact만 사용하도록 제한했습니다. Web과 MCP도 같은 deterministic Core 읽기 경로를 사용해 별도의 process truth를 만들지 않습니다.", en: "I constrained AI to facts returned by five read-only tools instead of calculating authoritative metrics. Web and MCP use the same deterministic Core read path rather than creating another process truth." } },
        ...
        architecture: {
          ...
          stages: [
            { name: "Browser import / dataset contract", copy: { ko: "명시적 mapping·검증", en: "explicit mapping and validation" } },
            { name: "Python / DuckDB deterministic Core", copy: { ko: "authoritative metrics", en: "authoritative metrics" } },
            { name: "5 read-only tools", copy: { ko: "공유 Core facts", en: "shared Core facts" } },
            { name: "FastAPI / React + Agent / MCP", copy: { ko: "같은 read path", en: "same read path" } },
            { name: "BPIC12 Power BI export", copy: { ko: "별도 역사적 분석 표면", en: "separate historical surface" } },
          ],
